...
module.exports = function (queryObj, options) {
    ...
    return function processImages(assetGraph, cb) {
        var getFilterInfosAndTargetContentTypeFromQueryString = require('@one.com/express-processimage/lib/getFilterInfosAndTargetContentTypeFromQueryString');
        ...
        async.each(Object.keys(isAvailableByBinaryName), function (binaryName, cb) {
            if (binaryName === 'jpegtran' || binaryName === 'optipng' || binaryName === 'pngcrush' || binaryName === 'pngquant') {
                ({jpegtran: JpegTran, optipng: OptiPng, pngcrush: PngCrush, pngquant: PngQuant})[binaryName].getBinaryPath(function (err, binaryPath) {
                    if (err) {
